            ...
            <nav role="navigation" aria-label="Pagination Navigation" class="items-center justify-between sm:flex">
                <div class="flex justify-center mt-2 md:flex-none md:justify-end sm:mt-0">
                    ...
                    @if(!$paginator->onFirstPage())
                        <a
                            class="px-2 py-1 pt-2 m-1 text-center text-white bg-slate-500 border-slate-400 cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800 dark:text-slate-300"
                            wire:click="gotoPage(1)"
                        >
                            <x-livewire-powergrid::icons.chevron-double-left/>
                        </a>

                        <a class="px-2 py-1 pt-2 m-1 text-center text-white bg-slate-500 border-slate-400 cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800 dark:text-slate-300"
                        ...
                        @if (is_array($element))
                            ...
                            @foreach ($element as $page => $url)
                                ...
                                @if ($page == $paginator->currentPage())
                                    <span
                                        class="px-2 py-1 m-1 text-center border-slate-400 cursor-pointer border-1 dark:bg-slate-700 dark:text-white dark:text-slate-300">{{ $page }}</span>
                                @elseif ($page === $paginator->currentPage() + 1 || $page === $paginator->currentPage() + 2 || $page === $paginator->currentPage() - 1 || $page === $paginator->currentPage() - 2)
                                    <a class="px-2 py-1 m-1 text-center text-white bg-slate-500 border-slate-400 cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800 dark:text-slate-300"
            ...
            @if ($paginator->hasPages() && $recordCount == 'min')
                <nav role="navigation" aria-label="Pagination Navigation" class="items-center justify-between sm:flex">
                    ...
                    <span>
                        ...
                        @if ($paginator->onFirstPage())
                            <button disabled
                                    class="p-2 m-1 text-center text-slate-400 bg-slate-200 border-slate-400 border-1 dark:text-slate-300">
                                <x-livewire-powergrid::icons.chevron-double-left/>
                            </button>
                        @else
                            @if(method_exists($paginator,'getCursorName'))
                                <button
                                    ...
                                    class="p-2 m-1 text-center text-white bg-slate-500 border-slate-400 cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800 dark:text-slate-300">
                                        <x-livewire-powergrid::icons.chevron-double-left/>
                                </button>
                            @else
