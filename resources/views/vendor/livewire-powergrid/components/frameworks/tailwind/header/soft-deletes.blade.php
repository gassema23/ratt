@if(data_get($setUp, 'header.softDeletes'))
    ...
    <div x-data="{open: false}"
        ...
        <button @click.prevent="open = ! open"
                class="block bg-slate-50 text-slate-700 border border-slate-300 py-1.5 px-3 leading-tight
        ...
        <div x-show="open"
            ...
            <div x-on:click="$wire.emit('pg:softDeletes-{{ $tableName }}', ''); open = false"
                 class="cursor-pointer flex justify-start block px-4 py-2 text-slate-800 hover:bg-slate-50 hover:text-black-200 dark:text-slate-200 dark:hover:bg-slate-900 dark:hover:bg-slate-700">
