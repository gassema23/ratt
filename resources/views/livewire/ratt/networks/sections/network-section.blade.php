    ...
    <div class="flex justify-end w-full pb-4 align-middle content-center items-center">
        @can('networks-edit')
            <a href="#" class="text-teal-500 hover:underline uppercase text-xs"
                wire:click="$emit('openModal', 'ratt.networks.edit',{{ json_encode([$network->id]) }})">
                <x-icon name="pencil" class="w-4 h-4 inline-block" /> @lang('Edit')
            </a>
        @endcan
    </div>
    <x-app-card :title="$network->name">
        ...
        <div wire:loading.remove>
            <div class="grid grid-cols-1 lg:grid-cols-6 gap-4">
                <div class="col-span-4">
                    <div class="font-bold">@lang('Description')</div>
                    <p class="text-slate-500">{{ $network->description }}</p>
                    @can('comments-view')
                        <div class="my-4">
                    ...
                    <div class="mb-4">
                        ...
                        <ul class="flex flex-col divide-y divide-slate-200 text-xs">
                            ...
                            <li class="inline-flex items-center gap-x-2 py-1.5 px-2 font-medium text-slate-500">
                                ...
                                {{ $network->site->city->name }} / {{ $network->site->name }} / {{ $network->site->clli }}
                            </li>
                            {{--
                            <li class="inline-flex items-center gap-x-2 py-1.5 px-2 font-medium text-slate-500">
