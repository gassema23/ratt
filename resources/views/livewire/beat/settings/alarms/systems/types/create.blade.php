<div>
    <x-app-modal>
        <form wire:submit.prevent="save">
            <x-slot name="title">
                {{ trans('New alarm system type') }}
            </x-slot>
            <x-slot name="content">
                <x-errors class="my-2 " />
                <div class="my-2 grid grid-cols-2 gap-4">
                    <x-input wire:model.defer="label.en" :label="trans('System type name [EN]')" :hint="trans('Field required')" />
                    <x-input wire:model.defer="label.fr" :label="trans('System type name [FR]')" />
                </div>
